...
function isDuplicate(fromUserId: string, text: string): boolean {
...
function buildEndpoint(accountId: string, target: string): string {
...
function formatInboundMessage(
  ...
  if (mediaResult.decryptedPicPath) parts.push(`\n[image: ${mediaResult.decryptedPicPath}]`);
  if (mediaResult.decryptedVideoPath) parts.push(`\n[video: ${mediaResult.decryptedVideoPath}]`);
  if (mediaResult.decryptedFilePath) parts.push(`\n[file: ${mediaResult.decryptedFilePath}]`);
  if (mediaResult.decryptedVoicePath) parts.push(`\n[voice: ${mediaResult.decryptedVoicePath}]`);
...
function startBridgeServer(): void {
  const server = http.createServer(async (req, res) => {
    if (req.method === "POST" && req.url === "/send") {
      ...
      req.on("end", async () => {
        try {
          const data = JSON.parse(body) as { to: string; content: string; media_path?: string; account_id?: string };
          ...
          if (!accountId) {
            ...
            if (to.startsWith("account:")) {
              ...
              if (pipe !== -1) accountId = rest.slice(0, pipe);
            }
          }
          if (!accountId) {
